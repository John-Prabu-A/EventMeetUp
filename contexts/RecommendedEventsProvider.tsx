...
export const RecommendedEventsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const fetchRecommendedEvents = useCallback(async () => {
        ...
        const { data: embeddingData, error: userEmbeddingError } = await supabase.from("profiles").select("user_embedding").eq("id", user.id).single();
    ...
    return (
        <RecommendedEventsContext.Provider value={{ recommendedEvents, loading, error, fetchRecommendedEvents }}>
            {children}
